...
1. 노트북과 `src/hangul\_font\_diffusion\_mvp.py`를 준비합니다.
2. 위 링크에서 폰트를 받아 Drive 폴더에 넣습니다.
3. Colab에서 노트북을 열고 GPU 런타임으로 설정합니다.
4. 셀을 순서대로 실행합니다. Cell 3에서 `ROOT`/`SRC` 경로, Cell 6에서 `MY\_FONT\_DIR`을 본인 환경에 맞게 수정하세요.
